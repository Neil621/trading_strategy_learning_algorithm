...
def test_code():
    ...
    for impact, _ in df_cum_ret.iterrows():
        print("Comparing cumulative return against impact={}".format(impact))
    ...
    plt.title("Cumulative return against impact on {} stock over in-sample (training) period".format(symbol))
    plt.xlabel("Impact")
    ...
    fig, ax = plt.subplots()
    nb_orders[["Benchmark"]].plot(ax=ax, color="b", marker="o")
    nb_orders[["Manual Strategy"]].plot(ax=ax, color="r", marker="o")
    nb_orders[["QLearning Strategy"]].plot(ax=ax, color="g", marker="o")
